...
export interface ImagesProps {
  setMedias: React.Dispatch<React.SetStateAction<string[]>>
}

const Images = ({ setMedias }: ImagesProps) => {
  ...
  const handleDeleteImage = (image: MediaResponse) => {
    ...
    setImagesPayload((prev) => prev.filter((item) => item !== image.url))
  }
  // Cập nhật images khi có sự thay đổi để gọi api chính xác
  useEffect(() => {
    setMedias(imagesPayload)
  }, [imagesPayload, setMedias])
