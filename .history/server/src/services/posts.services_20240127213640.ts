...
class PostsService {
  ...
  async getPosts(query: QueryReqBody) {
    ...
    postsWithUserInfo.sort((a, b) => {
      const isAVip = postsVip.some((vipPost) => vipPost.post_id === a.post._id)
      console.log('PostsService ~ postsWithUserInfo.sort ~ isAVip:', isAVip)
      const isBVip = postsVip.some((vipPost) => vipPost.post_id === b.post._id)
      console.log('PostsService ~ postsWithUserInfo.sort ~ isBVip:', isBVip)
