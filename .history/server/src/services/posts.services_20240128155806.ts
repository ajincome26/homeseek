...
class PostsService {
  ...
  async getPosts(query: QueryReqBody) {
    ...
    console.log('PostsService ~ getPosts ~ province:', province)
    ...
    if (province !== '') {
      conditions.address = { $regex: new RegExp(province, 'i') }
      console.log('PostsService ~ getPosts ~ address:', conditions.address)
    }
