...
import { OverviewBonus, OverviewStatus, SortOrder } from '~/constants/enums'
...
class PostsService {
  ...
  async getAllPost(query: QueryReqBody) {
    ...
    let sortField: 'createdAt' | 'price' | 'acreage' = 'createdAt'
    let sortOrder: -1 | 1 = 1
    ...
    const posts = await databaseService.posts.find().sort(sortObject).toArray()
    //
