...
const RegisterPage = () => {
  ...
  useEffect(() => {
    ...
    console.log('file: RegisterPage.tsx:49 ~ useEffect ~ new_user:', new_user)
    if (access_token && refresh_token) {
      setIsLoggedIn(true)
      setUserInfo({ email, name, phone_number, avatar, account_balance, username } as User)
      setUserInfoToStorage({ email, name, phone_number, avatar, account_balance, username } as User)
    ...
    registerMutation.mutate(data, {
      ...
      onError: (error) => {
        ...
        if (isAxiosUnprocessableEntityError<ResponseError<FormRegister>>(error)) {
          ...
          // formError sẽ có kiểu là XXX (kiểu truyền vào ResponseError)
          if (formError) {
