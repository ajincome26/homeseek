...
import { ErrorMSGType, ResponseError } from '~/types/utils.type'
  ...
  const handleSignUp = (data: FormRegister) => {
    registerMutation.mutate(data, {
      ...
      onError: (error) => {
        ...
        if (isAxiosUnprocessableEntityError<ResponseError<>>(error)) {
          const formError = error.response?.data.errors
          // formError sẽ có kiểu là XXX (kiểu truyền vào ResponseError)
          if (formError) {
            Object.keys(formError).forEach((key) => {
              const { msg } = formError[key as keyof FormRegister]
