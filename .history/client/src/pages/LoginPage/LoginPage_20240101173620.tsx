...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    const avatar = params.get('avatar') as string
    ...
    if (access_token && refresh_token) {
      setIsLoggedIn(true)
      setUserInfo({ email, name, phone_number, avatar } as User)
      setUserInfoToStorage({ email, name, phone_number, avatar } as User)
      setAccessToken(access_token as string)
      setRefreshToken(refresh_token as string)
      if (new_user == '1' && location.pathname == '/login/oauth') {
  ...
  }, [location, location.pathname, navigate, params, setIsLoggedIn, setUserInfo])
