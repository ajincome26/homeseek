...
import { orderP as orderConstP, orderA as orderConstA, sortBy } from '~/constants/post'
...
const List = () => {
  ...
  const handleNavigatePrice = (orderValue: Exclude<PostListParams['order'], undefined>) => {
    return navigate({
      pathname: path.HOME,
      search: createSearchParams({
        ...queryParamsConfig,
  ...
  const handleNavigateAcreage = (orderValue: Exclude<PostListParams['order'], undefined>) => {
    return navigate({
      pathname: path.HOME,
      search: createSearchParams({
        ...queryParamsConfig,
      ...
      <div className='flex items-center gap-3 mt-3'>
        ...
        <div className='relative'>
          ...
          >
            ...
            <option value={orderConstP.ASC} className='bg-white text-primary'>
              Thấp đến cao
            </option>
            <option value={orderConstP.DESC} className='bg-white text-primary'>
              Cao đến thấp
            </option>
          </select>
        ...
        <div className='relative'>
          <select
            ...
            value={order}
            onChange={(e) => handleNavigateAcreage(e.target.value as Exclude<PostListParams['order'], undefined>)}
          >
            <option value='' className='bg-white text-primary' disabled>
              Diện tích
            </option>
            <option value={orderConstA.ASC} className='bg-white text-primary'>
              Thấp đến cao
            </option>
            <option value={orderConstA.DESC} className='bg-white text-primary'>
              Cao đến thấp
            </option>
          </select>
          <div className='absolute top-1/2 right-4 translate-y-[-50%] cursor-pointer'>
