...
const { ImSearch, HiOutlineBuildingOffice, MdOutlinePlace, TbReportMoney, BiCrop, HiOutlineBackspace, FiChevronRight } =
  icons

const Modal = ({ setIsShowModal, content, name, title, handleSubmit, queries, arrMinMax }: ModalProps) => {
  const handleActive = () => {}
  const handleBeforeSubmit = () => {}
  return (
    <div className='fixed top-0 left-0 z-10 w-full h-full bg-overlay-60 text-secondary' onClick={handleOnClick}>
      <div
        className='w-[35%] bg-secondary bg-opacity-[85] text-white relative left-1/2 top-[10%] translate-x-[-50%] rounded-lg'
        // ref={nodeRef}
      >
        ...
        {(name === 'category' || name === 'province') && (
          <div className='flex flex-col gap-3 px-5 py-3'>
            ...
            {content?.map((item) => (
              <div key={item.code} className='flex items-center gap-3 pb-3 search-item-border'>
                <input
                  type='radio'
                  name={name}
                  id={item.code}
                ...
                />
                <label htmlFor={item.code} className='text-lg cursor-pointer cr-hover-text-yellow'>
                  {item.value}
                </label>
              </div>
            ))}
