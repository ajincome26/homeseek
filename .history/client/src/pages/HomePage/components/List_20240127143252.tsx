...
const List = () => {
  ...
  useEffect(() => {
    if (postsQuery.data?.data.result === 0) {
      navigate({
        pathname: path.HOME,
  ...
  console.log('List ~ postsQuery.data?.data.result.posts:', postsQuery.data?.data.result)
    ...
    <div>
      {postsQuery.data && (
        <div className='border border-[#ccc] p-3 text-primary rounded-md shadow-md'>
          ...
          {/* <div className='posts'>{postsQuery.data.data.result.posts?.map((item) => <PostItem key={item.title} />)}</div> */}
        </div>
      )}
    </div>
  )
