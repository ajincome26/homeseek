...
import { ArrMinMax, SearchItemType } from '~/types/other.type'
...
const Modal = ({ setIsShowModal, data, name, title, handleSubmit, queries, arrMinMax }: ModalProps) => {
  ...
  const handleBeforeSubmit = () => {
    ...
    let arrMinMax = [conver100toTarget(min), conver100toTarget(max)]
    // const gaps =
    //   name === "price"
    ...
    handleSubmit(
      {
        ...
        [name]: `Từ ${conver100toTarget(min)} - ${conver100toTarget(max)} ${name === 'price' ? 'triệu' : 'm2'}`
      },
      {
        [`${name}Arr`]: [min, max]
      }
