...
const { HiStar, BsSuitHeart, BsSuitHeartFill } = icons;
...
const PostItem = ({
  ...
  const handleStar = (star) => {
    ...
    for (let i = 1; i <= +star; i++)
      stars.push(
        <HiStar className="inline-block pb-[6px]" size={20} color="#fdc318" />
      );
    return stars;
  };
  return (
    <div className="relative flex w-full gap-2 pt-3 my-3 border-t border-t-red-600">
      <Link
        ...
        className="w-2/5 grid grid-cols-2 gap-[1px] relative cursor-pointer cr-hover-images"
      >
          ...
          images
            ...
            ?.map((i, index) => (
              <img
                ...
                src={i}
                alt="preview"
                className="object-cover w-full h-[127px]"
              />
            ))}
        {/* Có thể dùng cách 2 */}
        ...
        >
          {isHoverHeart ? (
            <BsSuitHeartFill color="#f73859" size={24} />
          ) : (
            <BsSuitHeart color="white" size={24} />
          )}
        </span>
      </Link>
      <div className="flex flex-col w-3/5">
        ...
        <div className="flex items-center justify-between my-[10px]">
          <span className="font-semibold text-secondary basis-2/6">
            {attributes?.price}
          </span>
          <span className="basis-1/6">{attributes?.acreage}</span>
          <span
        ...
        <p className="font-normal text-fifth cr-truncate-4">{description}</p>
        <div className="flex items-center justify-between mt-auto">
          <div className="flex items-center basis-1/2">
            <div className="mr-2 overflow-hidden rounded-full">
              <img
                src="https://i.imgur.com/SipxhNX.jpg"
                alt=""
