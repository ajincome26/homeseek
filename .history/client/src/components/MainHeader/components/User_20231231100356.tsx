...
const User = () => {
  ...
  return (
    <div className='flex items-center justify-center gap-2 mr-10 cursor-pointer'>
      ...
      <div>
        <span className='text-[17px] font-semibold text-primary'>{(userInfo as User)?.name || 'Anonymous'}</span>
        <div className='flex items-center gap-2'>
          <span className='font-medium text-primary'>
            {`Số Điện Thoại: `}
            <span className='font-semibold'>{(userInfo as User)?.phone_number || '0866516936'}</span>
          </span>
