...
const User = () => {
  ...
  return (
    <div className='flex items-center justify-center gap-2 mr-10 cursor-pointer'>
      ...
      <div>
        <span className='text-[17px] font-semibold text-secondary'>{currentData?.name}</span>
        <div className='flex items-center gap-2'>
          <span className='font-medium text-secondary'>
            {`Mã tài khoản: `}
            <span className='font-semibold'>{currentData?.id?.split('-')[4].slice(-5).toUpperCase()}</span>
          </span>
