...
const Button = ({ type, children, isLoading, to, className, hover, ...rest }: ButtonProps) => {
    ...
    return (
      <Link to={to || ''}>
        <button
          type={type || 'button'}
          disabled={isLoading}
          className={`${className} flex items-center justify-center text-[#3d3d3d] font-semibold transition duration-300 ease-linear bg-secondary rounded-md ${
            hover &&
  ...
  return (
    <button
      type={type || 'button'}
      disabled={isLoading}
      className={`${className} flex items-center justify-center font-semibold text-[#3d3d3d] transition duration-300 ease-linear bg-secondary rounded-md ${
        hover &&
        'relative after:absolute after:top-0 after:left-0 after:w-full after:h-full hover:after:bg-[#000] hover:after:bg-opacity-20 after:transition-all after:ease-linear after:duration-200'
      }`}
      {...rest}
