...
import { SearchItemType, GenderType } from '~/types/other.type'
  ...
  return (
    <div className={`flex flex-col gap-2 ${type === 'category' || type === 'target' ? 'w-full' : 'basis-[30%]'}`}>
      <select
        ...
        value={value}
        onChange={handleChange} // value của select là value của option (ở đây là province_id)
      >
        ...
        {type === 'province'
          ...
          : type === 'district'
            ...
            : type === 'category'
              ? categories?.map((item: SearchItemType) => (
